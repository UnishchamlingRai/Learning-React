...
function ReservationForm({bookedDates,cabin,user}) {
  ...
  return (
    <div className='scale-[1.01]'>
      <div className='bg-primary-800 text-primary-300 px-16 py-2 flex justify-between items-center'>
        <p>Logged in as</p>
        <p>{formatDate(range?.from)} to {formatDate(range?.to)}</p>
      ...
      <form action={async (formatData)=>{
       ...
       resetRange()
        ...
        <div className='flex justify-end items-center gap-6'>
          {!(range.from && range.to)?<p className='text-primary-300 text-base'>Start by selecting dates</p>

          
          :<SubmitButton padingLabel="Reserving..." > Reserve now</SubmitButton>}
        </div>
